...
if __name__=='__main__':
    ...
    unet_model.compile(optimizer="Adam", loss="mae",
                       metrics=["mean_absolute_error", "mean_squared_error"]) #compiling the U-Net
    unet_model.summary() #visulaizing the layers and its order
    callbacks = [ReduceLROnPlateau(factor=0.1, patience=5, min_lr=0.00001, verbose=1),
                 ModelCheckpoint(filepath = os.path.join(bestmodel_dir,
                                 'saved_model.epoch_{epoch:02d}-MSE_{val_mean_squared_error:.5f}.h5'),
